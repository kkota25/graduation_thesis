/************************************************************
 * インドネシア ADM2 × 2020年 の integrated deforestation alerts (ha)
...
// ----------------------------------------------------------
// 5. Date バンドを用いて 2020年のアラート面積 (ha) 画像を作成
//    Date: 2019-01-01 を 0 日目とした通し日数
//    2020年 → 365〜729 日（start=365, end=730）
// ----------------------------------------------------------
var dateBand = integrated.select('Date');

function makeAreaImageForYear(year) {
  year = ee.Number(year);
  var start = year.subtract(2019).multiply(365); // 2019→0, 2020→365
  var end   = start.add(365);                   // 0〜364, 365〜729, ...
...
// 2020年について ADM2 ごとに ha を合計
function aggregateAlertsByAdm2(year) {
  year = ee.Number(year);   // ここでは常に 2020
...
// ----------------------------------------------------------
// 6. 2020年のみ ADM2×年（=ADM2×2020）を集計
// ----------------------------------------------------------
var years = ee.List([2020]);
var perYear = years.map(aggregateAlertsByAdm2);
var allYears = ee.FeatureCollection(perYear).flatten();

// デバッグ用 print はコメントアウト
// print('ADM2 × 2020 alerts sample:', allYears.limit(10));

// ----------------------------------------------------------
// 7. Google Drive に CSV としてエクスポート（2020年のみ）
// ----------------------------------------------------------
Export.table.toDrive({
  collection: allYears,
  description: 'idn_integrated_alerts_adm2_yearly_2020',
  fileFormat: 'CSV'
});
