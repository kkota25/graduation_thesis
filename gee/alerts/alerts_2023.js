/************************************************************
 * インドネシア ADM2 × 2023年 の integrated deforestation alerts (ha)
...
// ----------------------------------------------------------
// 5. Date バンドを用いて 2023年のアラート面積 (ha) 画像を作成
//    Date: 2019-01-01 を 0 日目とした通し日数
//    2023年 → 1460〜1824 日（start=1460, end=1825）
// ----------------------------------------------------------
var dateBand = integrated.select('Date');

function makeAreaImageForYear(year) {
  year = ee.Number(year);
  var start = year.subtract(2019).multiply(365); // 2023→1460
...
// 2023年について ADM2 ごとに ha を合計
function aggregateAlertsByAdm2(year) {
  year = ee.Number(year);   // ここでは常に 2023
...
// ----------------------------------------------------------
// 6. 2023年のみ ADM2×年（=ADM2×2023）を集計
// ----------------------------------------------------------
var years = ee.List([2023]);
var perYear = years.map(aggregateAlertsByAdm2);
var allYears = ee.FeatureCollection(perYear).flatten();

// デバッグ用 print はコメントアウト
// print('ADM2 × 2023 alerts sample:', allYears.limit(10));

// ----------------------------------------------------------
// 7. Google Drive に CSV としてエクスポート（2023年のみ）
// ----------------------------------------------------------
Export.table.toDrive({
  collection: allYears,
  description: 'idn_integrated_alerts_adm2_yearly_2023',
  fileFormat: 'CSV'
});
