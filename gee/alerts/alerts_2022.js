/************************************************************
 * インドネシア ADM2 × 2022年 の integrated deforestation alerts (ha)
...
// ----------------------------------------------------------
// 5. Date バンドを用いて 2022年のアラート面積 (ha) 画像を作成
//    Date: 2019-01-01 を 0 日目とした通し日数
//    2022年 → 1095〜1459 日（start=1095, end=1460）
// ----------------------------------------------------------
var dateBand = integrated.select('Date');

function makeAreaImageForYear(year) {
  year = ee.Number(year);
  var start = year.subtract(2019).multiply(365); // 2019→0, 2020→365, 2021→730, 2022→1095
  var end   = start.add(365);                   // 各年 365 日分（end は排他的）
...
// 2022年について ADM2 ごとに ha を合計
function aggregateAlertsByAdm2(year) {
  year = ee.Number(year);   // ここでは常に 2022
...
// ----------------------------------------------------------
// 6. 2022年のみ ADM2×年（=ADM2×2022）を集計
// ----------------------------------------------------------
var years = ee.List([2022]);
var perYear = years.map(aggregateAlertsByAdm2);
var allYears = ee.FeatureCollection(perYear).flatten();

// デバッグ用 print はコメントアウト
// print('ADM2 × 2022 alerts sample:', allYears.limit(10));

// ----------------------------------------------------------
// 7. Google Drive に CSV としてエクスポート（2022年のみ）
// ----------------------------------------------------------
Export.table.toDrive({
  collection: allYears,
  description: 'idn_integrated_alerts_adm2_yearly_2022',
  fileFormat: 'CSV'
});
