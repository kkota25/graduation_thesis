/************************************************************
 * インドネシア ADM2 × 2024年 の integrated deforestation alerts (ha)
...
// ----------------------------------------------------------
// 5. Date バンドを用いて 2024年のアラート面積 (ha) 画像を作成
//    Date: 2019-01-01 を 0 日目とした通し日数
//    2024年 → 1825〜2189 日（start=1825, end=2190）
// ----------------------------------------------------------
var dateBand = integrated.select('Date');

function makeAreaImageForYear(year) {
  year = ee.Number(year);
  var start = year.subtract(2019).multiply(365); // 2024→1825
...
// 2024年について ADM2 ごとに ha を合計
function aggregateAlertsByAdm2(year) {
  year = ee.Number(year);   // ここでは常に 2024
...
// ----------------------------------------------------------
// 6. 2024年のみ ADM2×年（=ADM2×2024）を集計
// ----------------------------------------------------------
...
// print('ADM2 × 2024 alerts sample:', allYears.limit(10));

// ----------------------------------------------------------
// 7. Google Drive に CSV としてエクスポート（2024年のみ）
// ----------------------------------------------------------
Export.table.toDrive({
  collection: allYears,
  description: 'idn_integrated_alerts_adm2_yearly_2024',
  fileFormat: 'CSV'
});
