...
## ダイナミック・パネル 2SLS (ln(alerts+1)=0を除外)
アラートが発令されてない場所は森林じゃない？→除外
...
とすると、Assunção と Burgessのサンプル構築思想をかなり忠実に踏まえた「森林のある自治体サンプル」になります。
...
```{r 2SLS_first_stage_lag robust, echo=FALSE}
fs_ln_alerts_l1 <- feols(
  ln_alerts_l1 ~ cloud_share_l1 + chirps_mm + burned_ha |
    adm2_code + year,
  data = reg_dyn_frontier_loose,
...
```{r 2SLS_second_stage_lag robust, echo=FALSE}
iv_defor_l1 <- feols(
  defor_rate ~ chirps_mm + burned_ha |      # 外生コントロール（当期）
    adm2_code + year |                      # 固定効果
    ln_alerts_l1 ~ cloud_share_l1,          # 内生 ~ IV（ともに1期ラグ）
