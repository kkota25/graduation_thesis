...
$$
defor\_rate_{it}
= \beta_1 \ln(alerts_{i,t-1}+1)
+ \beta_2 \,chirps\_mm_{it}
+ \beta_3 \,burned\_ha_{it}
+ \alpha_i + \lambda_t + u_{it}.
$$

```{r Baseline_OLS_lag, echo=FALSE}
ols_defor_l1 <- feols(
  defor_rate ~ ln_alerts_l1 + chirps_mm + burned_ha |
    adm2_code + year,
  data    = reg_lag,
  cluster = ~adm2_code
)

summary(ols_defor_l1)
```

### 解釈(Baseline FEOLS(ラグあり))
-ln_alerts_l1：-0.00020（p≈0.25）
  - 符号はわずかに負に変わるが、有意ではない。
  -「前年 alerts が多いほど翌年のdefor_rateが低いかもしれない」程度の示唆だが、ノイズレベル。
- chirps_mm：-0.0000021（p≈0.043）
  -ここでも負で 5% 有意。降水量の効果は仕様を変えても安定。
- burned_ha：ほぼ 0（p≈0.99）
→ ラグを入れても alerts の係数は有意にならず、当年・ラグいずれの仕様でも
　「alerts と defor_rate の関係ははっきりしない」が結論。
